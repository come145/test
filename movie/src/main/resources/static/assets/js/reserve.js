...
//DOMContentLoaded 이벤트 = html의 문서를 완전히 불러오고 분석했을때 발생함
...
function addDate() {
    ...
    year = date.getFullYear(); // 현재 년도를 출력하는 메소드
    month = date.getMonth() + 1;  // 현재 월을 출력하는 메소드 , 0부터시작 0 = 1월
    
    reserveDate.append(year + '/' + month);  // 컨텐츠를 선택된 요소 내부의 끝에 삽입
  	// 선택 날짜와 달 append
  
  
  	// getDate() = 오늘날짜(ex : 1월27일이면 ) = 27
    for (i = date.getDate(); i <= lastDay.getDate(); i++) {
        
        // createElement = 지정한 tagName의 html요소를 만들어 반환함
        ...
        const dayOfWeek =
            weekOfDay[new Date(year + '-' + '0' +month + '-' + (i<10 ? "0" + i : i)).getDay()];
