...
<%@page import="com.movie.vo.TicketVO"%>
...
<link rel="stylesheet"
	href="assets/css/fonts/material-design-iconic-font/css/material-design-iconic-font.min.css">
<!-- MATERIAL DESIGN ICONIC FONT -->

<style type="text/css">

input[type="button"]:disabled {
    background-image: url('/assets/img/X표시.jpg'); 
...
<body>
	...
	<script type="text/javascript">
	 const button = document.querySelectorAll('seat');
			...
			<div class="select-seat-container">
				...
				<div class="select-seat-information">
					...
					<div class="select-seat-information-wrapper">
						<div class="select-theater-place selected-theater-place-info">${reserve.selectedTheater}</div>
						<div class="select-theater-place selected-theater-place-info">3관</div>
						<div class="select-theater-place">
							<span>남은좌석</span><span class="remain-seats">172</span>/<span class="all-seats">172</span>
						</div>
					</div>
					...
					<form action="/moveKakao.do" class="seatForm" method="post">
						...
						<button type="button" class="reserve-button">
							결제하기<img src="/assets/img/payment_icon_yellow_medium.png">
						</button>
					</form>
				</div>
...
const seatWrapper = document.querySelector('.seat-wrapper');
							//  querySelelctor =  html의 클래스 이름, 태그를 갖고 오기 위해 사용함
let clicked = '';
let div = '';
let seat = document.querySelectorAll('.seat');
					//querySelectorAll = 특정 css 선택자를 가진 요소를 배열로 가져오는 메서드
...
<c:forEach items="${seat}" var="item1">

seat1.push("${item1}");

</c:forEach>

let selectedItem = document.getElementsByClassName("seat").value;
...
for (let i = 0; i < 10; i++) {
    ...
    for (let j = 0; j < 12; j++) {
        ...
        for (let k = 0; k < seat1.length; k++) {


			seatArray.push(selectedItem);
...
const set = new Set(seatArray);


const uniqueArr = [...set];  // 중복제거한 시트 전체값
 							 // seat1 = 관객이 고른 시트

  	
let set1 = uniqueArr.filter(x => seat1.includes(x));


for(let i=0; i < uniqueArr.length; i++) {

	
		if(uniqueArr[i] = set1[i]) {	

			
			$('.seat[value*=' +uniqueArr[i]+ ']').attr('disabled', true);
